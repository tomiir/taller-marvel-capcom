...
JsonDisconection JsonParser:: getJsonDisconection(){
    ...
    if( !(json_["z-index"]).isInt() || (json_["z-index"]).asInt() < 0){
        error = true;
        errors.push_back("z-index");
    }
    ...
    else
    {
        ...
        int z_index = (json_["z_index"]).asInt();
        return JsonDisconection(name,path,x,y,height,width,z_index);
